...
\item Output: a file containing \emph{n} lines in the following format:
  \texttt{X R$_1$(n$_1$) R$_2$(n$_2$) R$_3$(n$_3$) ...} where \texttt{R$_i$} are the ids of the friends recommended to user \texttt{X} and $n_i$
  is the number of friends in common between \texttt{X} and
...
\item Map step:
  ...
  \end{itemize}
  In other words, \texttt{(a,b)} \emph{and} \texttt{(b,a)} are emitted every time a friend common to user \texttt{a} and user \texttt{b} is found. The following key-value pairs are emitted in response to the input example above:
...
\item Reduce step:
  ...
  \item Receive (k, [v]): \texttt{X, [ F$_1$, F$_2$
    ...
    when users \texttt{X} and \texttt{F$_i$} have \texttt{x}
...
Implement the algorithm described above. Make sure that it produces
the correct result using the example above. Then, test your
implementation on larger networks by generating input files using a
program such
 as \href{https://github.com/glatard/big-data-analytics-course/raw/master/labs/pymk/generate.py}{\texttt{generate.py}}.
...
recommend to user X people that are already friend with X. For
instance, in the example above, user \texttt{3} is recommended to user
\texttt{1} although \texttt{1} and \texttt{3} are already friends. We
need to somehow emit key-value pairs containing the friends of user
\texttt{i}, to address this issue.  Here is a possible solution:
\begin{itemize}
\item Map step:
  \begin{itemize}
  ...
  \item Emit (k,v):
    ...
    \item \texttt{X}, \color{red}\textbf{-}\color{black}\texttt{F$_i$} for all $i \in [1,k]$ 
    \end{itemize}
    (Note the \texttt{-} sign). For instance, in the example above: \textfile{pymk/emitted-1.txt}
  \end{itemize}
\item Reduce step:
  \begin{itemize}
  \item Receive (k, [v]): \texttt{X, [ F$_1$, F$_2$, ... ]} \newline
    
    If \texttt{F$_i$} is negative then it is a friend of user \texttt{X}. Otherwise, it is a user who has friends in common with \texttt{X}. In our example:
...
\item Output: A file containing \emph{n} lines in the following
  format: \texttt{X R$_1$(n$_1$) R$_2$(n$_2$) R$_3$(n$_3$) ...} where
